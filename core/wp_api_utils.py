...
def fetch_wordpress_posts(api_url, page=1):
    ...
    try:
        ...
        print(f"Ошибка при получении постов со страницы {page}: {e}")
        return [], 1
    ...
    try:
        ...
        if post and 'content' in post and 'rendered' in post['content']:
            ...
            try:
                ...
                print(f"Ошибка при декодировании контента в UTF-8 для поста с ID {post_id}: {e}")
                content = ""
            return content
        else:
            print(f"Предупреждение: Не удалось извлечь контент для поста с ID {post_id}")
            return ""  # Возвращаем пустую строку, чтобы избежать ошибок
    except requests.exceptions.RequestException as e:
        print(f"Ошибка при получении поста с ID {post_id}: {e}")
        return ""  # Возвращаем пустую строку в случае ошибки
    except json.JSONDecodeError as e:
        print(f"Ошибка при разборе JSON для поста с ID {post_id}: {e}")
        return ""
    except Exception as e:
        print(f"Непредвиденная ошибка при получении или обработке поста с ID {post_id}: {e}")
        return ""
