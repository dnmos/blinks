...
*   Скрипт `main.py` автоматически отправляет уведомление в Telegram с прикрепленным PDF-отчетом (если указаны API Token и Chat ID, в противном случае - выполняется заглушка).
