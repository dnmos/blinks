...
2.  **`tripster_link_processor.py`**:
    ...
    *   **Назначение:** Скрипт является вторым и **последним** шагом. Он обрабатывает данные о постах WordPress, собранные на первом шаге, и **извлекает из контента каждого поста виджеты и партнерские ссылки Tripster**. Затем он проверяет работоспособность партнерских ссылок (для диплинков) и сохраняет результаты.
    *   **Действия:**
        ...
        *   Для каждого поста:
            ...
            *   **Используя модуль `tripster_data_extractor.py`, извлекает из HTML-контента виджеты и партнерские ссылки Tripster. Для проверки статуса диплинков используется функция `check_deeplink_status_api` из модуля `core/tripster_api_utils.py`.**
...
*   **`tripster_link_processor.py`**:
    *   Входные данные:
        *   Переменные окружения: `TRIPSTER_DOMAIN`, `MAX_RETRIES`, `RETRY_DELAY` (определены в файле `.env`).
    ...
    *   Выходные данные:
        *   JSON файл `json/tripster_links.json`, содержащий информацию о виджетах и диплинках Tripster, найденных в постах WordPress.
        *   Лог-сообщения в консоль и в файл (если настроено).
    *   Ожидаемый формат выходных данных:
        ```json
        [
                ...
                "widgets": [
                    {
                        ...
                        "url": "https://example.com/excursion/456",
                ...
                "deeplinks": [
                    {
                        "id": 789,
                        "anchor": "Текст ссылки",
                        "url": "https://example.com/excursion/789",
                        "status": "active",
                        "title": "Название экскурсии",
                        "inactivity_reason": null,
                        "is_unknown_type": false
