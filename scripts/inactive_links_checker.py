    ...
    for i, post in enumerate(data):  # Итерируемся по всем постам
        ...
        for link_number, deeplink in enumerate(deeplinks, start=1):
            ...
            try:
                ...
                is_inactive = False
                inactivity_reason = ""
                is_unknown_type = False # Добавлено: Флаг для неизвестного типа
                ...
                inactive_links.append({
                    'link_number': link_number,
                    'reason': f"Ошибка при запросе URL: {e}",
                    'is_unknown_type': False  # Считаем, что это не неизвестный тип
                })
                total_inactive_links += 1
                ...
                inactive_links.append({
                    'link_number': link_number,
                    'reason': f"Ошибка при обработке URL: {e}",
                    'is_unknown_type': False  # Считаем, что это не неизвестный тип
                })
                total_inactive_links += 1
